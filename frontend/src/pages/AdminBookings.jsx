...
export default function AdminBookings() {
    ...
    return (
        <div>
                ...
                {FILTERS.map((f) => {
                    ...
                    return (
                        <button
                            key={f}
                            onClick={() => setFilter(f)}
                            className={`text-[10px] uppercase tracking-[0.3em] border px-3 py-1.5 transition-colors ${
                                activeFilter === f
                                    ? "border-foreground bg-foreground text-background"
                                    : "border-border hover:border-foreground"
                            }`}
                            data-testid={`admin-bookings-filter-${f}`}
                        >
                            {f} · {count}
                        </button>
                    );
            ...
            {visible.length === 0 ? (
                <p className="text-muted-foreground text-center py-12">No bookings in this view.</p>
            ) : (
                <div
